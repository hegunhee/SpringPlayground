...
public class BasicTxTest {
    ...
    void inner_rollback_requires_new() {
        ...
        log.info("내부 트랜잭션 시작");
        DefaultTransactionAttribute defination = new DefaultTransactionAttribute();
        defination.setPropagationBehavior(TransactionDefinition.PROPAGATION_REQUIRES_NEW);
        TransactionStatus inner = txManager.getTransaction(defination); // 기존의 트랜잭션은 잠시 suspending시켜놓음
        log.info("inner.isNewTransaction()={}",inner.isNewTransaction());

        log.info("내부 트랜잭션 롤백");
        txManager.rollback(inner); // 기존의 중지시킨 트랜잭션을 실행시킴
